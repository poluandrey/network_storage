...
class Network(Base):
    ...
    parent_id = Column(Integer, ForeignKey('network.id', ondelete='RESTRICT'), nullable=True)
    network = Column(String, unique=True)
    create_at = Column(DateTime(timezone=True), server_default=func.now())
    last_update = Column(DateTime(timezone=True), onupdate=func.now())

    sub_networks = relationship("Network", remote_side=[parent_id], back_populates='parent_network')
